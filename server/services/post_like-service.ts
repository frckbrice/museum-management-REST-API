...
/** Minimal payload for like/unlike; id and createdAt are generated by DB. */
type PostLikePayload = Pick<PostLike, "userId" | "postId">;

export class PostLikeService extends BaseService {
  async postLike(data: PostLikePayload) {
    try {
  ...
  async unlikePost(data: PostLikePayload) {
    try {
      validateId(data?.userId, "user id");
      validateId(data?.postId, "post id");
      return this.withErrorHandling(async () => {
