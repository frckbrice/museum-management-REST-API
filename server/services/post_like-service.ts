...
export class PostLikeService extends BaseService {
  async postLike(data: PostLike) {
    try {
      ...
      console.error(error);
      throw new Error(
        ` error creating like for post id : ${data?.postId} created by user id : ${data?.userId}`
      );
    }
  }
    ...
    try {
      ...
      return this.withErrorHandling(async () => {
        const result = await this.db
          .delete(postLikes)
          .where(and(eq(postLikes.userId, data.userId), eq(postLikes.postId, data.postId)))
          .returning();
        return result || null;
      }, "unlikePost");
    } catch (error) {
      console.error(error);
      throw new Error(
        ` error deleting like for post id : ${data?.postId} created by user id : ${data?.userId}`
      );
    }
  }
}
