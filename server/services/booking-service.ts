...
export class BookingService extends BaseService {
    ...
    async getBookingsByUserId(userId: string, limit: number = 20, offset: number = 0): Promise<{
        data: Booking[];
        total: number;
        hasMore: boolean;
    }> {
    ...
    async getAllBookings(
        ...
        return this.withErrorHandling(async () => {
            const whereCondition = status ? eq(bookings.status, status as typeof bookingStatus.enumValues[number]) : undefined;
