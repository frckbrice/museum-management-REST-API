...
export class ForumService extends BaseService {
    ...
    async getAllPosts(
        ...
        return this.withErrorHandling(async () => {
            ...
            const whereCondition = typeof isAttendantOnly === 'boolean'
    ...
    async getCommentsByPostId(postId: string, limit: number = 50, offset: number = 0): Promise<{
        data: Comment[];
        total: number;
        hasMore: boolean;
    }> {
    ...
    async createComment(data: Partial<Comment>): Promise<Comment> {
        ...
        return this.withErrorHandling(async () => {
            const [comment] = await this.db.insert(comments).values(sanitizedData as Comment).returning();
            return comment;
        }, 'createComment');
    }
}
